...
def convert_to_nighttime(image_path, output_path):
    ...
    hsv[:,:,2] = hsv[:,:,2] * 0.5  # Reduce brightness
...
def batch_convert_to_nighttime(input_dir, output_dir):
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            output_path = os.path.join(output_dir, f"night_{filename}")
...
batch_convert_to_nighttime( script_dir / "Images",  script_dir / "Night" / "night_dark")